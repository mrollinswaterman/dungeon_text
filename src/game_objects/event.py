...
class Event():
    ...
    def try_again(self, code) -> None:
        ...
        globals.type_text(random.choice(self._messages[False][code]))#print failure line

    def not_that_stat(self, code:str):
        """Runs if the player tries the event with the wrong stat """
        globals.type_text(random.choice(FAILURE_LINES[code]))#print failure line
